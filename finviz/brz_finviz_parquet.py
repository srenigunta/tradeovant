...
def transform_partition_parquet(
    ...
    conn = duckdb.connect()
    stg_path = rf"{source_partition_dir}\*.parquet"

    # Execute the custom SQL
    #arrow_table = conn.execute(query_select).arrow()
    arrow_table = conn.execute(f"{query_select} from read_parquet('{stg_path}', hive_partitioning = True)").arrow()
    ...
    out_file = os.path.join(target_partition_dir, f"brz_{file_name}.parquet")
...
def main(rerun_partitions):
    ...
    logging.info("Loading finviz_sql_config_brz.yaml ...")
    current_dir = os.path.dirname(__file__)
    sql_path = os.path.join(current_dir, "..", "config", "finviz_sql_config_brz.yaml")
    sql_config_file = os.path.abspath(sql_path)

    with open(sql_config_file, "r", encoding="utf-8") as f:
    ...
    for dataset_name, ds_conf in datasets.items():
        ...
        stage_sub_path = ds_conf["stage_sub_path"]  # e.g. "stage_store/finviz/fundamentals"
        bronze_sub_path = ds_conf["bronze_sub_path"]  # e.g. "bronze_store/finviz/fundamentals"
        partition_column = ds_conf.get("partition_column", "file_version_date")
        query_select = ds_conf["query_select"]
        # print(query_select)
        ...
        bronze_path = os.path.join(storage.BASE_PATH, bronze_sub_path)

        logging.info(f"Stage path: {stage_path}")
        logging.info(f"Bronze path: {bronze_path}")

        # List partitions in source (stage) and target (bronze)
        source_partitions = list_partitions_in_dir(stage_path, partition_column)
        target_partitions = list_partitions_in_dir(bronze_path, partition_column)

        logging.info(f"Found {len(source_partitions)-len(target_partitions)} partitions in stage for {dataset_name} to be processed...")
        ...
        for pval in source_partitions:
            ...
            if do_rerun:
                ...
                remove_partition_dir(bronze_path, pval, partition_column)
